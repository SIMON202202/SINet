...
class GraphConvolution(nn.Module):
    ...
    def forward(self, A, X):
        ...
        """
        # X = torch.mm(X, self.W)  # 特徴量の線形変換
        # X = torch.spmm(A, X)    # 周辺ノードからの情報収集 (aggregation)

        X = X.transpose(0, 1)

        # X = self.dp(X)
        X = torch.matmul(X, self.W)  # 特徴量の線形変換
        X = torch.matmul(X.transpose(0, 2), A)  # 周辺ノードからの情報収集 (aggregation)
...
class GCN(nn.Module):
    ...
    def forward(self, A, X):
        ...
        x = self.dp(x)
        x = self.act(self.bn1(self.l1(x)))  # グラフ特徴量を線形変換する # [N, P*D2]
        x = self.dp(x)
        x = self.act(self.bn2(self.l2(x)))  # グラフ特徴量を線形変換する # [N, P*D2]
        x = self.dp(x)
        x = self.l3(x)  # グラフ特徴量を線形変換する # [N, P*D2]

        '''
        x = self.dp(x)
        x = self.act(self.l1(x))  # グラフ特徴量を線形変換する # [N, P*D2]
        x = self.dp(x)
        x = self.act(self.l2(x))  # グラフ特徴量を線形変換する # [N, P*D2]
        # x = self.act(self.l3(x))  # グラフ特徴量を線形変換する # [N, P*D2]
        '''
...
class GCN_loading(nn.Module):
    ...
    def forward(self, A, X):
        ...
        # return torch.sigmoid(x)  # 二値分類なので最終層の活性化関数は sigmoid 関数
