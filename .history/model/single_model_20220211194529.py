...
class Proto(nn.Module):
    ...
    def append(self, train_rmse, valid_rmse, in_rmse, in_pehe, in_ate, in_ks, in_vio, out_rmse, out_pehe, out_ate, out_ks, out_vio):
        ...
        self.df = self.df.append({'method': self.args.model, 'expid': self.args.expid,
                                  ...
                                  'within_rmse': in_rmse, 'within_pehe': in_pehe, 'within_ate': in_ate,
                                  'within_ks': in_ks, 'within_vio': in_vio,
                                  'without_rmse': out_rmse, 'without_pehe': out_pehe, 'without_ate': out_ate,
                                  'without_ks': out_ks, 'without_vio': out_vio}, ignore_index=True)
        # ------------------------------ #
        '''
        # ------------------------------ #
        # 最新の結果を読み込む
    ...
    def _get_score(self, testloader, plot=False, limit=False):
        ...
        for (i, data) in enumerate(pbar):
            ...
            # -------------------------------- #
            # ytest *= self.y_scaler.data_max_.max()
            # ypred_test *= self.y_scaler.data_max_.max()
            # mtest *= self.y_scaler.data_max_.max()
            # -------------------------------- #
            #ytest *= scale.squeeze().numpy()
            #ypred_test *= scale.squeeze().numpy()
            # -------------------------------- #
            ...
            if i == 0:
                plt.clf()
                plt.plot(ytest[:30].T, 'k', alpha=0.4)
                plt.plot(ypred_test[:30].T, 'b', alpha=0.4)
                plt.title('test')
                plt.savefig('tmp_test.png')
            '''
            # mtest = mtest.detach().cpu().numpy()
