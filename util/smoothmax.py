...
class MySmoothMax(torch.autograd.Function):
    ...
    def forward(ctx, output, target):
        ...
        _max, _ = x.max(1, keepdim=True)
        x = x / _max
        exp = torch.exp(self.alpha * x)
        loss = (x*_max*exp).sum(1)/exp.sum(1)
